...
if access == 'Yes':
    ...
    lab_occ = st.number_input("Occupancy (%):", min_value=0, max_value=100, value=None, help="Estimate of what percentage of the room the lab equipment occupies",key='lab_occ')
    ...
    grid_asset = st.columns([0.11,0.0625,0.0625,0.0625,0.08,0.1,0.08,0.07,0.1,0.11,0.08,0.115])
    ...
    def add_row_asset(row):
        ...
        with grid_asset[2]:
            ...
            if row == 0:
                asset_rm[row]=st.number_input('Room \n \n No.', min_value=0, max_value=None, value=None, step = 1, key=f'input_colb_b{row}',)
            else:
                asset_rm[row]=st.number_input('Temp', min_value=0, max_value=None, value=None, step = 1, key=f'input_colb_b{row}', label_visibility = "collapsed", )
        # -- Asset Age
        with grid_asset[3]:
